...
function msx_card_insert_video(attachment) {
  ...
  jQuery("#cards").append('<div class="msx-card" id="' + attachment.id + '"><div class="card-controls"><span id="controls_' + attachment.id + '"><span class="dashicons dashicons-format-video"></span> ' + attachment.title + '</span><a class="card-delete" href="#" onclick="msx_card_delete( ' + attachment.id + ' )" title="Delete card"><span class="dashicons dashicons-dismiss" style="float:right"></span></a></div><img id="card_img_' + attachment.id + '" src="' + $image + '" style="max-width:300px;max-height:185px;margin:0 auto;"><p><input type="text" size="32" class="card-title" name="card_' + attachment.id + '_title" id="card_' + attachment.id + '_title" value="' + attachment.title + '" placeholder="Title" /><br><textarea cols="32" name="card_' + attachment.id + '_content" id="card_' + attachment.id + '_content" placeholder="Caption">' + attachment.caption + '</textarea></p><span class="dashicons dashicons-admin-links" style="float:left;padding:5px;"></span><input type="text" size="28" name="cards_' + attachment.id + '_link" id="card_' + attachment.id + '_link" value="" placeholder="Link" /><input type="hidden" name="card_' + attachment.id + '_format" id="card_' + attachment.id + '_format" value="video" /></div>');
  $order_new = jQuery("#cards_order").val();
  $order_new = $order_new + ',' + attachment.id;
  $order_new = $order_new.replace(",,",",");
  jQuery("#cards_order").val($order_new);
